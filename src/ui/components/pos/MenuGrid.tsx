...
export const MenuGrid: React.FC = () => {
  const { menuItems, activeCategory, setActiveCategory, addToCart } = useStore();
    ...
    <>
      <div className="menu-grid-scroll">
        {filteredItems.length === 0 ? (
          <div className="kds-empty-state" style={{ gridColumn: '1 / -1', padding: '3rem 1rem' }}>
            <AlertTriangle size={36} style={{ color: 'var(--accent-amber)', marginBottom: '0.5rem' }} />
            <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#ffffff' }}>No Items Found</h3>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', maxWidth: '280px', margin: '0.35rem 0 1rem' }}>
              There are no dishes currently mapped to the &ldquo;{activeCategory}&rdquo; category.
            </p>
              ...
              <div key={item.id} className={`pos-card ${is86 ? 'pos-card-disabled' : ''}`}>
                ...
                >
                  <img
                    ...
                    loading="lazy"
                    onError={(e) => {
                      e.currentTarget.src = '/restaurant_logo.jpg';
                    }}
                  />
                  {is86 && (
